...
std::string build_medaka_command(const std::filesystem::path &combined_reads,
                                 ...
                                 const bacpipe::MedakaConfig &tool,
    ...
    command << "mkdir -p " << bacpipe::shell_quote(medaka_dir.string()) << " && "
            << bacpipe::shell_quote(tool.executable)
            << " -i " << bacpipe::shell_quote(combined_reads.string())
            << " -d " << bacpipe::shell_quote(draft_assembly.string())
            << " -o " << bacpipe::shell_quote(medaka_dir.string())
            << " -t " << threads
...
void validate_medaka_config(const bacpipe::MedakaConfig &medaka) {
...
std::vector<PipelineStep> build_medaka_steps(const PipelineConfig &config) {
