...
bool uses_auto_genome_size(const bacpipe::AutocyclerConfig &autocycler) {
    return autocycler.genome_size == "auto";
}

std::string genome_size_argument(const bacpipe::AutocyclerConfig &autocycler,
...
std::string build_combine_reads_command(const std::vector<std::filesystem::path> &trimmed_reads,
    ...
    command << mkdir_parent_command(combined_reads)
            << " && (for read in";
...
std::string build_genome_size_command(const std::filesystem::path &combined_reads,
                                      const std::filesystem::path &genome_size_path,
                                      const bacpipe::AutocyclerConfig &tool,
    ...
    command << mkdir_parent_command(genome_size_path) << " && "
            ...
            << " --threads " << threads
            << " > " << bacpipe::shell_quote(genome_size_path.string());
...
std::string build_subsample_command(const std::filesystem::path &combined_reads,
    ...
    command << mkdir_parent_command(subsampled_reads_dir) << " && "
            ...
            << " --genome_size " << genome_size_argument(tool, genome_size_path)
            << " --count " << tool.subsample_count
...
std::string build_helper_command(const std::filesystem::path &subsampled_read,
    ...
    command << mkdir_parent_command(out_prefix) << " && "
            ...
            << " --out_prefix " << bacpipe::shell_quote(out_prefix.string())
            << " --threads " << threads
            << " --genome_size " << genome_size_argument(tool, genome_size_path)
            << " --read_type " << bacpipe::shell_quote(tool.read_type)
...
std::string build_compress_command(const std::filesystem::path &assemblies_dir,
    ...
    command << mkdir_parent_command(autocycler_dir) << " && "
            ...
            << " -i " << bacpipe::shell_quote(assemblies_dir.string())
...
std::string build_trim_resolve_command(const std::filesystem::path &autocycler_dir,
    ...
    command << "for c in " << bacpipe::shell_quote(qc_pass_dir.string()) << "/cluster_*; do "
            ...
            << " -t " << threads
            << bacpipe::join_shell_args(tool.trim_extra_args)
            << " && " << bacpipe::shell_quote(tool.executable) << " resolve -c \"$c\""
            << bacpipe::join_shell_args(tool.resolve_extra_args)
            << " || exit 1; done";
...
std::string build_combine_command(const std::filesystem::path &autocycler_dir,
    ...
    command << bacpipe::shell_quote(tool.executable) << " combine"
            << " -a " << bacpipe::shell_quote(autocycler_dir.string())
            << " -i " << bacpipe::shell_quote(qc_pass_dir.string()) << "/cluster_*/5_final.gfa"
            << bacpipe::join_shell_args(tool.combine_extra_args)
            << " && if [ " << bacpipe::shell_quote(native_consensus.string()) << " != "
...
void validate_autocycler_config(const bacpipe::AutocyclerConfig &autocycler) {
...
std::vector<PipelineStep> build_autocycler_steps(const PipelineConfig &config) {
    ...
    steps.push_back(PipelineStep{.name = "Combine trimmed reads for Autocycler and Medaka",
                                 .command =
                                     build_combine_reads_command(trimmed_reads, combined_reads),
    ...
    if (uses_auto_genome_size(config.autocycler)) {
        steps.push_back(PipelineStep{
            .name = "Estimate genome size for Autocycler",
    ...
    steps.push_back(PipelineStep{
        .name = "Subsample reads for Autocycler",
        .command =
            build_subsample_command(combined_reads,
                                    subsampled_reads_dir,
                                    config.autocycler,
                                    genome_size_path),
        .working_directory = config.project_root,
        .expected_outputs =
            expected_subsample_outputs(subsampled_reads_dir, config.autocycler.subsample_count),
        .skip_when_outputs_exist = true});
    ...
    steps.push_back(PipelineStep{
        .name = "Compress Autocycler input assemblies",
        .command =
            build_compress_command(assemblies_dir, autocycler_dir, config.autocycler, config.threads),
        .working_directory = config.project_root,
        .expected_outputs = {autocycler_dir / "input_assemblies.gfa"},
        .skip_when_outputs_exist = true});

    steps.push_back(PipelineStep{.name = "Cluster Autocycler input contigs",
                                 .command =
                                     build_cluster_command(autocycler_dir, config.autocycler),
                                 .working_directory = config.project_root,
    ...
    steps.push_back(PipelineStep{.name = "Combine Autocycler resolved clusters",
                                 .command = build_combine_command(autocycler_dir,
                                                                  consensus_fasta,
                                                                  config.autocycler),
                                 .working_directory = config.project_root,
